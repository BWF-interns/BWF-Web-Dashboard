    ...
    <div className="space-y-6">
      ...
      {error ? (
        <div className="card flex items-center gap-3 text-red-600 bg-red-50 border-red-200">
          <AlertCircle size={18} />
          <p className="text-sm">Failed to load complaints. Please refresh the page.</p>
        </div>
      ) : isLoading ? (
                  ...
                  {c.status === "OPEN" && (
                    <div className="flex flex-col gap-2 flex-shrink-0">
                      <button
                        ...
                        className="btn-success flex items-center gap-1.5 py-2 px-3 text-sm"
                      >
                        <CheckCircle size={15} /> Resolve
                      </button>
                      <button
                        onClick={() => handleAction(c._id, "escalate")}
                        className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-3 rounded-lg text-sm transition-colors"
                      >
