...
import { Student, Activity, Expense, Complaint } from "@/lib/types";
...
export default function StudentProfilePage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {tab === "Personal Info" && (
        <PersonalInfoTab student={student} editing={editing} setEditing={setEditing} onSave={mutate} />
      )}
      {tab === "Activities" && (
        activitiesError
          ? <ErrorMessage />
          : <ActivitiesTab activities={activitiesData?.activities || []} loading={!activitiesData} />
      )}
      {tab === "Expenses" && (
        expensesError
          ? <ErrorMessage />
          : <ExpensesTab expenses={expensesData?.expenses || []} loading={!expensesData} />
      )}
      {tab === "Complaints" && (
        complaintsError
          ? <ErrorMessage />
          : <ComplaintsTab complaints={complaintsData?.complaints || []} loading={!complaintsData} />
      )}
    </div>
  );
}
...
function PersonalInfoTab({ student, editing, setEditing, onSave }: {
  ...
  // Reset form when student data changes (e.g. navigating between profiles)
  useEffect(() => {
    setForm({
      name: student.name, age: String(student.age), education: student.education || "",
      address: student.address || "", is_active: student.is_active,
    ...
    <div className="card p-0 overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-50">
          {activities.length === 0 ? (
            <tr><td colSpan={4} className="text-center py-8 text-gray-400">No activities</td></tr>
          ) : activities.map((a) => (
            <tr key={a._id}>
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-50">
          {expenses.length === 0 ? (
            ...
            <tr key={e._id}>
              <td className="px-6 py-3"><span className="badge-open">{e.category}</span></td>
...
function ComplaintsTab({ complaints, loading }: { complaints: Complaint[]; loading: boolean }) {
  ...
  return (
    <div className="space-y-3">
      {complaints.length === 0 ? (
        <div className="card text-center text-gray-400 py-8">No complaints</div>
      ) : complaints.map((c) => (
