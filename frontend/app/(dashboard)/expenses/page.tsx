...
import { Expense, Student } from "@/lib/types";
    ...
    <div className="space-y-6">
        ...
        {["", ...CATEGORIES].map((c) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              filterCategory === c ? "bg-blue-600 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
            }`}
          >
            {c || "All"}
          </button>
      ...
      {error ? (
        <div className="card flex items-center gap-3 text-red-600 bg-red-50 border-red-200">
          <AlertCircle size={18} />
          <p className="text-sm">Failed to load expenses. Please refresh the page.</p>
        </div>
      ) : isLoading ? (
              ...
              {expenses.length === 0 ? (
                ...
                return (
                  <tr key={e._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4"><span className="badge-open">{e.category}</span></td>
